...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-5xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold tracking-tight">Edit Distribution Rule <span
                    class="text-indigo-600">#{{ $rule->id }}</span></h1>
        </div>
        ...
        <form action="{{ route('distribution-rules.update', $rule->id) }}" method="POST" id="ruleForm" class="space-y-6">
            ...
            <div class="bg-white shadow-lg rounded-xl border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex justify-between items-center">
                    ...
                    <div
                        class="text-xs font-medium text-slate-500 bg-slate-100 px-3 py-1 rounded-full border border-slate-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                        <div class="col-span-1 md:col-span-6">
                            ...
                            <input type="text" name="name"
                                ...
                                required value="{{ old('name', $rule->name) }}" placeholder="e.g. Meta Ads High Priority">
                        </div>
                        <div class="col-span-1 md:col-span-3">
                            ...
                            <input type="number" name="priority"
                                class="form-input w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm"
                                min="1" required value="{{ old('priority', $rule->priority) }}">
                        ...
                        <div>
                            ...
                            <select name="project_id"
                                ...
                                @foreach($projects as $p)
                                    <option value="{{ $p->id }}" {{ $rule->project_id == $p->id ? 'selected' : '' }}>
                                        {{ $p->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="lead_source_id"
                                ...
                                @foreach($sources as $s)
                                    <option value="{{ $s->id }}" {{ $rule->lead_source_id == $s->id ? 'selected' : '' }}>
                                        {{ $s->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="fallback_behavior"
                                ...
                                <option value="skip" {{ $rule->fallback_behavior == 'skip' ? 'selected' : '' }}>Skip to next
                                    member</option>
                            </select>
